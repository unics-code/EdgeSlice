...
def drawResult(r,im,q):
    for i in range(len(r)):
        ...
        if ((x3<=im.shape[0]) and (y3>=0)):
            cv2.putText(im, str(r[i][0]), (x3,y3), font, 1, (0,255,0) , 3)
        else:
            cv2.putText(im, str(r[i][0]), (int(x1),int(y1+6)), font, 1, (0,255,0) , 3)
...
def display(q):
    ...
    while True:
        frameimg = q.get()
        coun += 1
        cv2.imwrite("/home/nvidia/Desktop/frames/image%.3d.png" %coun,frameimg)
        #out.write(frameimg)
        cv2.imshow('Haoxin Detection Window', frameimg)
...
if __name__ == "__main__":
    ...
    cap = cv2.VideoCapture('/home/nvidia/Desktop/DJI_20190401_115617.avi')
    q = Queue.LifoQueue()
    threads = []
    t1 = threading.Thread(target = readImg,args=(cap,q))
    threads.append(t1)
    ...
    #out = cv2.VideoWriter('/home/nvidia/Desktop/output.avi',fourcc, 24.0, (720,480))
    ...
    while True:
        if not q.empty():
            #ret, frame = cap.read() 
            frame = q.get()
            result = detect1(net, meta, frame, thresh=0.7)
            count+=1
            print 'count: ',count
