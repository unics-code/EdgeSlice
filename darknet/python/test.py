...
def detect(net, meta, image, quato=50, thresh=.5, hier_thresh=.5, nms=.45):
    im = load_image(image, 0, 0)
    num = c_int(0)
    pnum = pointer(num)
    predict_image(net, im, quato) #int(np.random.uniform(10,100))
    ...
    for j in range(num):
        for i in range(meta.classes):
            if dets[j].prob[i] > 0:
                b = dets[j].bbox
                classid = i
                res.append((meta.names[i], dets[j].prob[i], (b.x, b.y, b.w, b.h), classid))
    res = sorted(res, key=lambda x: -x[1])
    free_image(im)
...
def showPicResult(r,image):
    ...
    for i in range(len(r)):
        ...
        cv2.putText(im, text, (int(x1),int(y1)), cv2.FONT_HERSHEY_SIMPLEX,1, color, 2,cv2.LINE_AA)
...
if __name__ == "__main__":
    ...
    out_img = b"/home/nvidia/darknet/test/test_result.jpg"
    while(True):
        StartTime = time.time()
        r = detect(net, meta, b"data/dog.jpg")
        CompuLatency = time.time() - StartTime
        print ("round-trip latency %f " %CompuLatency)
        print (r)
        #showPicResult(r,"data/dog.jpg")
