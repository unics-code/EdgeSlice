...
NUM_SLICE = 3
HOST=''
USER_PORT=[9001, 9002, 9003, 9004]
CTL_PORT=11111
BUFFER_SIZE = 256
QUATO = [50, 50, 50]
num_points = 3
Latency = [[] for _ in range(NUM_SLICE)]
Count = 0
...
def connect_controller():
    ...
    while True:
        ...
        while True:
            ...
            #print(data)
            QUATO = [int(idata) for idata in data[:NUM_SLICE]]
            print('GPU virtual resource is ', QUATO)
            # wait for latency fill up
            while( not if_sufficient_data(Latency) ):
                time.sleep(0.01)
            send_data = np.ones(BUFFER_SIZE, dtype=np.double)
            send_data[:NUM_SLICE] = np.mean(Latency, axis=1)
            controller.sendall(send_data)

def if_sufficient_data(Latency):
    indicator = True
    for latency in Latency:
        if len(latency) < num_points:
            indicator = False
    return indicator
...
def detect(net, meta, image, index, thresh=.5, hier_thresh=.5, nms=.45):
    global QUATO
    ...
    num = c_int(0)
    pnum = pointer(num)
    predict_image(net, im, QUATO[index])
...
def threading_darknet(detect_net, detect_meta, client, index):
    print ("entered threading.", index)

    StartTime = time.time()

    while True:
        decimg = recv_image_from_socket(client)
        result = detect(detect_net, detect_meta, decimg, index, thresh=0.7)
        # Latency[index].append(time.time() - StartTime) # save the latency with the index
        print(index, "----",str(time.time() - StartTime))
        time.sleep(2) # wait 2 second so that the cpu load will not so high since we parallel run multiple model in single CPU
        str1 = '0'+'\n'
...
if __name__ == "__main__":
    ...
    detect_nets, detect_metas = [], []
    # load the detect network
    detect_nets.append(load_net("./cfg/yolov3-tiny.cfg", "yolov3-tiny.weights", 0))
    detect_nets.append(load_net("./cfg/yolov3-416.cfg", "yolov3.weights", 0))
    detect_nets.append(load_net("./cfg/yolov3-608.cfg", "yolov3.weights", 0))
    # load the detect meta
    detect_metas.append(load_meta("cfg/coco.data"))
    detect_metas.append(load_meta("cfg/coco.data"))
    detect_metas.append(load_meta("cfg/coco.data"))


    index = 0 # the index of accepted client

    while True:
        # setup the server socket
        s=socket.socket(socket.AF_INET,socket.SOCK_STREAM)
        s.bind((HOST,USER_PORT[index]))
        s.listen(10)
        # once the client is accepted
        client,addr=s.accept()
        print ("Get new user socket", index)
        # start the thread darknet
        threads = threading.Thread(target=threading_darknet, args=(detect_nets[index],detect_metas[index], client, index))
