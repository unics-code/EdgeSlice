...
__global__ void levels_image_kernel(float *image, float *rand, int batch, int w, int h, int train, float saturation, float exposure, float translate, float scale, float shift, int ix, int iy, int iz)
{
...
__global__ void forward_crop_layer_kernel(float *input, float *rand, int size, int c, int h, int w, int crop_height, int crop_width, int train, int flip, float angle, float *output, int ix, int iy, int iz)
{
	int bx = blockIdx.x + ix;
	int by = blockIdx.y + iy;
	int bz = blockIdx.z + iz;
		...
		for (int iz=0;iz<total_blocks.z;iz++)
		{
			for (int iy=0;iy<total_blocks.y;iy++)
			{
				...
				for (ix=0;ix<int(total_blocks.x/quato);ix++)
				    levels_image_kernel<<<quato, BLOCK>>>(net.input_gpu, layer.rand_gpu, layer.batch, layer.w, layer.h, net.train, layer.saturation, layer.exposure, translate, scale, layer.shift, ix*quato, iy, iz);
				...
				if (double(total_blocks.x) - quato * int(double(total_blocks.x)/quato) > 0)
				    levels_image_kernel<<<total_blocks.x - quato * ix, BLOCK>>>(net.input_gpu, layer.rand_gpu, layer.batch, layer.w, layer.h, net.train, layer.saturation, layer.exposure, translate, scale, layer.shift, ix*quato, iy, iz);
			}
		}
	}
	else
	{
	    levels_image_kernel<<<cuda_gridsize(size), BLOCK>>>(net.input_gpu, layer.rand_gpu, layer.batch, layer.w, layer.h, net.train, layer.saturation, layer.exposure, translate, scale, layer.shift, 0, 0, 0);
	}
	...
	if (VIRTUALGPU)
	{
		...
		for (int iz=0;iz<total_blocks.z;iz++)
		{
			for (int iy=0;iy<total_blocks.y;iy++)
			{
				...
				for (ix=0;ix<int(total_blocks.x/quato);ix++)
				    forward_crop_layer_kernel<<<quato, BLOCK>>>(net.input_gpu, layer.rand_gpu, size, layer.c, layer.h, layer.w, layer.out_h, layer.out_w, net.train, layer.flip, radians, layer.output_gpu, ix*quato, iy, iz);

				// if the iteration is not integer, run last time with fixed number blocks
				if (double(total_blocks.x) - quato * int(double(total_blocks.x)/quato) > 0)
				    forward_crop_layer_kernel<<<total_blocks.x - quato * ix, BLOCK>>>(net.input_gpu, layer.rand_gpu, size, layer.c, layer.h, layer.w, layer.out_h, layer.out_w, net.train, layer.flip, radians, layer.output_gpu, ix*quato, iy, iz);
			}
		}
	}
	else
	{
	    forward_crop_layer_kernel<<<cuda_gridsize(size), BLOCK>>>(net.input_gpu, layer.rand_gpu, size, layer.c, layer.h, layer.w, layer.out_h, layer.out_w, net.train, layer.flip, radians, layer.output_gpu, 0, 0, 0);
	}
