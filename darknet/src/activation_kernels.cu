...
__global__ void binary_gradient_array_kernel(float *x, float *dy, int n, int s, BINARY_ACTIVATION a, float *dx, int ix, int iy, int iz)
{
...
extern "C" void binary_gradient_array_gpu(float *x, float *dx, int n, int size, BINARY_ACTIVATION a, float *y) 
{
	if (VIRTUALGPU)
	{
		dim3 total_blocks = cuda_gridsize(n/2); int quato=QUATO;
		...
		for (int iz=0;iz<total_blocks.z;iz++)
		{
			for (int iy=0;iy<total_blocks.y;iy++)
			{
				...
				for (ix=0;ix<int(total_blocks.x/quato);ix++)
					binary_gradient_array_kernel<<<quato, BLOCK>>>(x, dx, n/2, size, a, y, ix*quato, iy, iz);
				...
				if (double(total_blocks.x) - quato * int(double(total_blocks.x)/quato) > 0)
					binary_gradient_array_kernel<<<total_blocks.x - quato * ix, BLOCK>>>(x, dx, n/2, size, a, y, ix*quato, iy, iz);
			}
		}
	}
	else
	{
		binary_gradient_array_kernel<<<cuda_gridsize(n/2), BLOCK>>>(x, dx, n/2, size, a, y, 0, 0, 0);
	}

	//binary_gradient_array_kernel<<<cuda_gridsize(n/2), BLOCK>>>(x, dx, n/2, size, a, y);
    check_error(cudaPeekAtLastError());
}
__global__ void binary_activate_array_kernel(float *x, int n, int s, BINARY_ACTIVATION a, float *y, int ix, int iy, int iz)
{
	int bx = blockIdx.x + ix;
	int by = blockIdx.y + iy;
	int bz = blockIdx.z + iz;
	...
	if (VIRTUALGPU)
	{
		...
		for (int iz=0;iz<total_blocks.z;iz++)
		{
			for (int iy=0;iy<total_blocks.y;iy++)
			{
				...
				for (ix=0;ix<int(total_blocks.x/quato);ix++)
					binary_activate_array_kernel<<<quato, BLOCK>>>(x, n/2, size, a, y, ix*quato, iy, iz);

				// if the iteration is not integer, run last time with fixed number blocks
				if (double(total_blocks.x) - quato * int(double(total_blocks.x)/quato) > 0)
					binary_activate_array_kernel<<<total_blocks.x - quato * ix, BLOCK>>>(x, n/2, size, a, y, ix*quato, iy, iz);
			}
		}
	}
	else
	{
		binary_activate_array_kernel<<<cuda_gridsize(n/2), BLOCK>>>(x, n/2, size, a, y, 0, 0, 0);
	}

    check_error(cudaPeekAtLastError());
}

__global__ void activate_array_kernel(float *x, int n, ACTIVATION a, int ix, int iy, int iz)
{
	int bx = blockIdx.x + ix;
	int by = blockIdx.y + iy;
	int bz = blockIdx.z + iz;
...
__global__ void gradient_array_kernel(float *x, int n, ACTIVATION a, float *delta, int ix, int iy, int iz)
{
	int bx = blockIdx.x + ix;
	int by = blockIdx.y + iy;
	int bz = blockIdx.z + iz;
...
extern "C" void activate_array_gpu(float *x, int n, ACTIVATION a) 
{
	if (VIRTUALGPU)
	{
		...
		for (int iz=0;iz<total_blocks.z;iz++)
		{
			for (int iy=0;iy<total_blocks.y;iy++)
			{
				...
				for (ix=0;ix<int(total_blocks.x/quato);ix++)
					activate_array_kernel<<<quato, BLOCK>>>(x, n, a, ix*quato, iy, iz);

				// if the iteration is not integer, run last time with fixed number blocks
				if (double(total_blocks.x) - quato * int(double(total_blocks.x)/quato) > 0)
					activate_array_kernel<<<total_blocks.x - quato * ix, BLOCK>>>(x, n, a, ix*quato, iy, iz);
	...
	{
		activate_array_kernel<<<cuda_gridsize(n), BLOCK>>>(x, n, a, 0, 0, 0);
	}
    check_error(cudaPeekAtLastError());
}
	...
	if (VIRTUALGPU)
	{
		...
		for (int iz=0;iz<total_blocks.z;iz++)
		{
			for (int iy=0;iy<total_blocks.y;iy++)
			{
				...
				for (ix=0;ix<int(total_blocks.x/quato);ix++)
					gradient_array_kernel<<<quato, BLOCK>>>(x, n, a, delta, ix*quato, iy, iz);

				// if the iteration is not integer, run last time with fixed number blocks
				if (double(total_blocks.x) - quato * int(double(total_blocks.x)/quato) > 0)
					gradient_array_kernel<<<total_blocks.x - quato * ix, BLOCK>>>(x, n, a, delta, ix*quato, iy, iz);
			}
		}
	}
	else
	{
		gradient_array_kernel<<<cuda_gridsize(n), BLOCK>>>(x, n, a, delta, 0, 0, 0);
	}
