...
GLOBAL_INFOS Global_Info={49,50};


void print_allocation_information(void)
{
	printf("**---------------------------------------------------------------------------**\n");
	int i=0;
	for (i=0;i<SLICENUM;i++)
	{
		if( Global_Info.Slice_Info[i].active == 1)
		{
			printf("slice %d, IMSI %d, RNTI %x, uRB %d, dRB %d \n",i, Global_Info.Slice_Info[i].IMSI, Global_Info.Slice_Info[i].RNTI, Global_Info.Slice_Info[i].uRB, Global_Info.Slice_Info[i].dRB); 
		}
	}
    	printf("**---------------------------------------------------------------------------**\n");
}
...
int socket_recv_function(int sockfd)
{
	printf("ready to recv the data ...\n"); 
	double recv_data[Global_Info.BUFFER_SIZE];
	if (recv(sockfd,recv_data, sizeof(recv_data),MSG_WAITALL)<=0) return 0;
	// TODO hard code here
	Global_Info.Slice_Info[0].uRB    =   recv_data[ 0 ];
	Global_Info.Slice_Info[0].dRB    =   recv_data[ 1 ];

	Global_Info.Slice_Info[1].uRB    =   recv_data[ 2 ];
	Global_Info.Slice_Info[1].dRB    =   recv_data[ 3 ];

	
	printf("recv resource allocation from hypervisor.\n");
	print_allocation_information();
	return 1;
}

void Initialization_(void)
{
	Global_Info.SOCKET_ID = 0;
	Global_Info.COMM_PORT = 8888;
	Global_Info.BUFFER_SIZE = 256;
	Global_Info.Thor = 1; // specify which algorithm it will use, Thor or Orignial 

	// manually set the initialization infos
	Global_Info.Slice_Info[0].UE_id = 0;
	Global_Info.Slice_Info[0].IMSI = 0;
	Global_Info.Slice_Info[0].RNTI = 0;
	Global_Info.Slice_Info[0].uRB = 10;
	Global_Info.Slice_Info[0].dRB = 10;
	Global_Info.Slice_Info[0].active = 1;  //TODO change the active to determine the slice status

	Global_Info.Slice_Info[1].UE_id = 1;
	Global_Info.Slice_Info[1].IMSI = 1;
	Global_Info.Slice_Info[1].RNTI = 0;
	Global_Info.Slice_Info[1].uRB = 10;
	Global_Info.Slice_Info[1].dRB = 10;
	Global_Info.Slice_Info[1].active = 1;  //TODO change the active to determine the slice status

	Global_Info.Slice_Info[2].UE_id = 2;
	Global_Info.Slice_Info[2].IMSI = 2;
	Global_Info.Slice_Info[2].RNTI = 0;
	Global_Info.Slice_Info[2].uRB = 10;
	Global_Info.Slice_Info[2].dRB = 10;
	Global_Info.Slice_Info[2].active = 1;  //TODO change the active to determine the slice status

	print_allocation_information();
}

int socket_server_build(int port)
{
	...
	// listen
	if (listen(server_socket, port)){printf("Server Listen Failed!\n");exit(1);}
	return server_socket;
}


void *recv_in_thread(void)
{
	while(1)
	{
		if (socket_recv_function(Global_Info.SOCKET_ID)!=1) break;// always recv the slice info, if recv broken, then break the while loop
		usleep(1000);
	}
	perror("slicing function down@@@@@@@@@@@@!!!!!!!!!!!!!!!!!!\n but we still wait for next connection from hypervisor.\n");

}

void *process_in_thread (void *arg)
{
	int server_socket = socket_server_build(Global_Info.COMM_PORT);
	struct sockaddr_in client_addr;
	socklen_t length = sizeof(client_addr);
	pthread_t recv_thread;

	while(1) // always listen the connection from the hypervisor
	{
		printf("waiting for connection at port %d under server socket %d\n",Global_Info.COMM_PORT,server_socket);
		Global_Info.SOCKET_ID = accept(server_socket, (struct sockaddr*)&client_addr, &length);
		if (Global_Info.SOCKET_ID < 0) perror("Server Accept Failed!\n");
		printf("hypervisor is connected with socket id %d!\n",Global_Info.SOCKET_ID);
		if(pthread_create(&recv_thread, NULL, recv_in_thread, NULL)==-1) perror("Thread creation failed");
	}
}

void start_radio_slicing_function(void)
{
	// initialization global information
	Initialization_();
	pthread_t process_thread;
	if(pthread_create(&process_thread, NULL, process_in_thread, NULL)==-1) perror("Thread creation failed");
	//process_in_thread(0);
}
