...
extern GLOBAL_INFOS Global_Info;

void print_slice_information_runtime(void)
{
	LOG_D(MAC,"**---------------------------------------------------------------------------**\n");
	int i=0;
	for (i=0;i<SLICENUM;i++)
	{
		LOG_D(MAC,"slice %d, IMSI %d, RNTI %x, uRB %d, dRB %d \n",i,  Global_Info.Slice_Info[i].IMSI, Global_Info.Slice_Info[i].RNTI, Global_Info.Slice_Info[i].uRB, Global_Info.Slice_Info[i].dRB); 
	}
    	LOG_D(MAC,"**---------------------------------------------------------------------------**\n");
}

int UE_RNTI_process_function_downlink(int UE_id)
{
	print_slice_information_runtime();
	LOG_D(MAC,"Search for downlink RB for UE_id %d\n",UE_id);
	int i;
	for (i=0;i<SLICENUM;i++)
	{
		if(Global_Info.Slice_Info[i].UE_id == UE_id && Global_Info.Slice_Info[i].active == 1) 
			return Global_Info.Slice_Info[i].dRB;
	}
	return 10; // 5 rb default
	// won't go here
}

int UE_RNTI_process_function_uplink(int UE_id)
{
	print_slice_information_runtime();
	LOG_D(MAC,"Search for uplink RB for UE_id %d\n",UE_id);
	int i;
	for (i=0;i<SLICENUM;i++)
	{
		if(Global_Info.Slice_Info[i].UE_id == UE_id && Global_Info.Slice_Info[i].active == 1)
			return Global_Info.Slice_Info[i].uRB;
	}
	return 10; // 5 rb default
	// won't go here
}
...
if (1) // This is Thor algorithm!!!!!!!!!!!
{
	if (total_ue_count > 0 ) {
		dlsch_my_scheduler_pre_processor( Mod_id,
						                  min_rb_unit,
						                  nb_rbs_required,
						                  nb_rbs_required_remaining,
						                  rballoc_sub,
...
{ //This is original downlink scheduling algorithm!!!!!!!!!!!!!!! 
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////////////////////////////////
void dlsch_my_scheduler_pre_processor (module_id_t   Mod_id,
    int           min_rb_unit[MAX_NUM_CCs],
    uint16_t      nb_rbs_required[MAX_NUM_CCs][NUMBER_OF_UE_MAX],
    uint16_t      nb_rbs_required_remaining[MAX_NUM_CCs][NUMBER_OF_UE_MAX],
    unsigned char rballoc_sub[MAX_NUM_CCs][N_RBG_MAX],
    unsigned char MIMO_mode_indicator[MAX_NUM_CCs][N_RBG_MAX])
{
	...
	int i,ii,k; int RBG_start_index=0; //int Kmax = cmin(slice_vector[UE_id],nb_rbs_required[CC_id][UE_id]);
	UE_list_t *UE_list = &eNB_mac_inst[Mod_id].UE_list;
    
	LOG_D(MAC,"calling dlsch my scheduler pre processor .. \n");
	for(i=UE_list->head; i>=0; i=UE_list->next[i]) {
		UE_id = i;
		ue_sched_ctl = &UE_list->UE_sched_ctrl[UE_id];
		rnti = UE_RNTI(Mod_id,UE_id);

		assigned_dRB = (UE_RNTI_process_function_downlink(UE_id)/3);//VRB is 2
		LOG_D(MAC,"assigned downlink rb of %x UE is %d\n",rnti,assigned_dRB);
		// LOG_D(MAC,"UE %d rnti 0x\n", UE_id, rnti );
		if(rnti == NOT_A_RNTI)
			continue;
		if (UE_list->UE_sched_ctrl[UE_id].ul_out_of_sync == 1)
		...
		for (ii=0; ii<UE_num_active_CC(UE_list,UE_id); ii++) {
			CC_id = UE_list->ordered_CCids[ii][UE_id];
			nb_rbs_required_remaining[CC_id][UE_id] = nb_rbs_required[CC_id][UE_id];// allocate the required RB of this UE at this CC
			int transmission_mode = mac_xface->get_transmission_mode(Mod_id,CC_id,rnti);
			if (transmission_mode == 5 ) { ue_sched_ctl->dl_pow_off[CC_id] = 1;}
			for (k=RBG_start_index; k<RBG_start_index + assigned_dRB; k++) { // loop all the slice_max until the RB is meet or over_index
				if (rballoc_sub[CC_id][k] == 0 && nb_rbs_required_remaining[CC_id][UE_id] > 0 ) { // make sure this RB and UE's RB_requirement have not been allocated and meet, respectively
					rballoc_sub[CC_id][k] = 1;
					ue_sched_ctl->rballoc_sub_UE[CC_id][k] = 1;
					MIMO_mode_indicator[CC_id][k] = 1;
					nb_rbs_required_remaining[CC_id][UE_id] = nb_rbs_required_remaining[CC_id][UE_id] - min_rb_unit[CC_id];
					ue_sched_ctl->pre_nb_available_rbs[CC_id] = ue_sched_ctl->pre_nb_available_rbs[CC_id] + min_rb_unit[CC_id];
				}
			}
		}
		RBG_start_index = RBG_start_index + assigned_dRB;
	}
}
////////////////////////////////////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
if (1) // This is Thor algorithm!!!!!!!!!!!
{
  ...
  for (i=UE_list->head_ul; i>=0; i=UE_list->next_ul[i]) {
	...
	assigned_uRB = UE_RNTI_process_function_uplink(i);//seems uplink dont have to divided by 2
	LOG_D(MAC,"assigned uplink rb of %x UE is %d\n",rnti,assigned_uRB);
    ...
    for (n=0; n<UE_list->numactiveULCCs[UE_id]; n++) {
      ...
      if(round>0) {
        ...
        nb_allocated_rbs[CC_id][UE_id] = cmin(UE_list->UE_template[CC_id][UE_id].pre_allocated_nb_rb_ul, assigned_uRB);
      }
	  LOG_D(MAC,"nb_allocated_rbs[%d] is %d which is cmin( %d, slice assigned RB size %d )\n",UE_id,nb_allocated_rbs[CC_id][UE_id], UE_list->UE_template[CC_id][UE_id].pre_allocated_nb_rb_ul,assigned_uRB);
  ...
  for(r=0; r<2; r++) {
	...
	continue;
      ...
      for (n=0; n<UE_list->numactiveULCCs[UE_id]; n++) {
		...
		/*
        if ( r == 0 ) {
		  // if there are spare RBs and the pre_allocated_nb_rb_ul not meet, then allocates 
...
{  // this is the orignal scheduling algorithm!!!!!!!!!!!!!
