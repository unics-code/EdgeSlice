...
def delete_METER_thread(*args):
  # args[0] = switch[s]
  # args[1] = int(m_l[u_index][1-top_num[u_index]][s][0]) <==this is the meter from the previous topology that needs to deleted b.c. the flows
  #connecting to this meter hav a higher priority then the 1st topology causing the data to flow through the flows with the higher priority
  flow.delete_meter(args[0],args[1])
...
thread2=[]
thread3=[] #<==this contains all of the meters for the previous topology that need to be deleted
for p in range(0,len(param)/2): #add a meter per user starting with base station 1's users
  ...
  while((ii<len(b1_u)) and (check==-1)):
    if b1_u[ii].find(str(param[h_count]))>=0:
      check=ii
      b1_u_mod[ii]=b1_u[ii]
      break
    ii=ii+1
  ...
  if(check<>-1):
    ...
    for s in swl: #goes through all of the switches with meters for this user
      ...
      thread3.append(threading.Thread(target=delete_METER_thread(switch[s],int(m_l[u_index][1-top_num[u_index]][s][0]))))
      ...
      f_l[u_index][top_num[u_index]] = flow.modify_f_l(f_l[u_index][top_num[u_index]],int(m_l[u_index][top_num][check][s][0]))
    #   f_l[u_index][1-top_num[u_index]] = flow.modify_f_l(f_l[u_index][1-top_num[u_index]],int(m_l[u_index][top_num][check][s][0]))
    i=thread_index
    while i<len(thread2):
...
# deletes the previous topologies meters
i=0
while i<len(thread3):
    # kills all threads if this python code is killed
    thread3[i].daemon = True
    # Starts the thread
    thread3[i].start()
    i=i+1
