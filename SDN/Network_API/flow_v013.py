...
# version 013 SUMMARY:
# The focus of this upgrade is too allow dynamic grouping in terms of IPv4 flows and only one time ARP paths creation, to reduce latency. Additional code was added
# to control ICMP packets as well since IPv4 is only controlling the TCP pakcets and ICMP flows control the UDP packets.



s_nums = [[0]]
# Write the ports in the format [source of packets port #, destination of packets port #]

ports = [    [[[1,4],[5,8],[9,12]]]  ] #TODO change this to the path desired for your topology
...
def add_flow_with_meter(*args):
    ...
    #2048 #0x0800
    print str(args)
    flow_type=['ARP','IPv4']#,'ICMP','TCP','UDP']	
    temp=np.zeros((int(len(flow_type))))
    #flow_rec[0] = flow #

    # This code is for ARP flows
    if(len(args)==9):
        if(int(args[8])==99):
    ...
    if(len(args)==9 and (int(args[8])==0 or int(args[8])==55)):
        for x in range(len(flow_type)):
            if flow_type[x]!='ARP':
                if int(args[8])==0:
                    # this code is for when a IPv4 or ICMP flow needs to be created
                    print "\nit was in 0"
                    if flow_type[x]=='IPv4':
                        temp[x]=add_flow_ipv4_with_meter(args[0],args[1],args[2],args[3],args[4],flow_num,ip_part,int(priority_part)-1)
                    elif flow_type[x]=='ICMP':
                        temp[x]=add_flow_icmp_with_meter(args[0],args[1],args[2],args[3],args[4],flow_num,ip_part,int(priority_part)-1)
                    elif flow_type[x]=='TCP':
                        temp[x]=add_flow_tcp_with_meter(args[0],args[1],args[2],args[3],args[4],flow_num,ip_part,int(priority_part)-1)
                    elif flow_type[x]=='UDP':
                        temp[x]=add_flow_udp_with_meter(args[0],args[1],args[2],args[3],args[4],flow_num,ip_part,int(priority_part)-1)
                elif int(args[8])==55:
                    print "\nit was in 55"
                    # this code is for when a group IPv4 or ICMP flow needs to be created
                    if flow_type[x]=='IPv4':
                        temp[x]=add_flow_ipv4_for_group_with_meter(args[0],args[1],args[2],args[3],args[4],flow_num,ip_part,int(priority_part)-1)
                    elif flow_type[x]=='ICMP':
                        temp[x]=add_flow_icmp_for_group_with_meter(args[0],args[1],args[2],args[3],args[4],flow_num,ip_part,int(priority_part)-1)
                    elif flow_type[x]=='TCP':
                        temp[x]=add_flow_tcp_for_group_with_meter(args[0],args[1],args[2],args[3],args[4],flow_num,ip_part,int(priority_part)-1)
                    elif flow_type[x]=='UDP':
                        temp[x]=add_flow_udp_for_group_with_meter(args[0],args[1],args[2],args[3],args[4],flow_num,ip_part,int(priority_part)-1)
...
def add_flow_tcp_for_group_with_meter(*args):
    global error
    #print(args) #debuging code
    #NOTES:
    #args[0] = 106225813884652 or 106225813884040 # the switch name
...
\t<table_id>0</table_id>\n</flow>"
    ...
    # response = R_ses.put(url, data=payload, headers=headers)
    response = requests.request("PUT", url, data=payload, headers=headers)
    print('DELTA TIME 4 Group TCP: '+str(time.time()-ttttt))
    #print(str(response.status_code)) #debuging code	
    #print(response.text) #debuging code
    # ttttt2= time.time()
    if ((response.status_code==200) or (response.status_code==201)):
        #print('TCP Flow entry was added to table 0 slot '+str(flow_num)+'.\n') #debuging code
        print('Group TCP Flow entry was added to connect port '+str(args[1])+' to '+str(args[2])+'.\n')
        #print('time for response_-------------------------------------'+str(time.time()-ttttt2))
        return(flow_num)
    else:
        flow_error.append('8')
        #print('time for rsponse_-------------------------------------'+str(time.time()-ttttt2))
        print('ERROR:Unable to add group tcp flow entry to table 0 slot '+str(flow_num)+'.\n')
        return(0)


def add_flow_icmp_for_group_with_meter(*args):
    global error 
    #print(args) #debuging code
    #NOTES:
    #args[0] = 106225813884652 or 106225813884040 # the switch name
...
\t<table_id>0</table_id>\n</flow>"
    ...
    if ((response.status_code==200) or (response.status_code==201)):
        #print('TCP Flow entry was added to table 0 slot '+str(flow_num)+'.\n') #debuging code
        print('Group ICMP Flow entry was added to connect port '+str(args[1])+' to '+str(args[2])+'.\n')
        return(flow_num)
    else:
        flow_error.append('9')
        print('ERROR:Unable to add group icmp flow entry to table 0 slot '+str(flow_num)+'.\n')
        return(0)
...
def add_flow_udp_for_group_with_meter(*args):
    global error
    #print(args) #debuging code
    #NOTES:
    #args[0] = 106225813884652 or 106225813884040 # the switch name
...
\t<table_id>0</table_id>\n</flow>"
    ...
    if ((response.status_code==200) or (response.status_code==201)):
        #print('TCP Flow entry was added to table 0 slot '+str(flow_num)+'.\n') #debuging code
        print('Group UDP Flow entry was added to connect port '+str(args[1])+' to '+str(args[2])+'.\n')
        return(flow_num)
    else:
        flow_error.append('10')
        print('ERROR:Unable to add group udp flow entry to table 0 slot '+str(flow_num)+'.\n')
...
def create_topology(*args):
    ...
    if top_num[u_index]==0 or top_num[u_index]==-1:
        ...
        #1            [input port]
        #2            [output port]
        #3            [defines this path as the users sending data (source=0) or the users recieving data (destination=1)]
        #4            [path #]
        sug_list = []
        ...
        for p in range(len(s_nums)): # <== path number
            for sn in range(len(s_nums[p])): # <== switch numbers
                for cn in range(len(ports[p][sn])):
                    ...
                    arp_list.append([s_nums[p][sn],ports[p][sn][cn][0],ports[p][sn][cn][1],0,p])

                    # eg adds the 1==>12 route
                    arp_list.append([s_nums[p][sn],ports[p][sn][cn][1],ports[p][sn][cn][0],1,p])
        ...
        while arp_counter<len(arp_list):
            if arp_list[arp_counter][0]!=(-1):
                ...
                # This look compares all of the routes to the current rout to see if their are any routes with the same input or output port as arp_list[arp_counter]
                temp_count=arp_counter+1
                while temp_count<len(arp_list):
                    if arp_list[temp_count][0]!=(-1):
                        # This code is for routes that have not been ruled out yet
                        ...
                        arp_list[arp_counter][2] == arp_list[temp_count][2]):
                            ...
                            print "IM in an if statement that shuold never occur if the user properly writes the \
                            information in the ports array correctly"
                            time.sleep(6000)
                            arp_list[temp_count][0]=-1
                            # this will add the path # for this route and if its route is meant for sending or recieving information
                            sug_list[arp_counter][6].append(arp_list[temp_count][4])
                            sug_list[arp_counter][5].append(arp_list[temp_count][3])
                ...
                if len(group_list)>1:
                    ...
                    if top_num[u_index]==1:
                        temp = add_flow_with_meter(\
                        switch[arp_list[arp_counter][0]],\
                        arp_list[arp_counter][1],\
                        group_list,\
                        '0.0.0.0',\
                        m_l[u_index][top_num[u_index]][switch_num][0],\
                        1,\
                        top_num[u_index],\
                        0,\
                        99)
    ...
    for u in range(0,len(b1_u)):
        if b1_u[u] != 'none':
            ...
            for c_path in range(len(sug_list)):
                b_num = 0#2*len(s_nums)-path_num #<== This will change when implemented for base station 1 too

                for path_num_index in range(len(sug_list[c_path][6])):
                    if int(sug_list[c_path][6][path_num_index]) in b1_paths: #<==This checks if the path # in the list of paths that need to be configured are required for b1 users.
                        ...
                        if sug_list[c_path][0] > 0 and sug_list[c_path][1] > 0:
                            ...
                            temp = add_flow_with_meter(\
                            switch[sug_list[c_path][2]],\
                            sug_list[c_path][3],\
                            group_num,\
                            b1_u[u],\
                            m_l[b_index][top_num[b_index]][sug_list[c_path][2]][0],\
                            sug_list[c_path][5][path_num_index],\
                            top_num[b_index],\
                            b_num,\
                            55)
                            # print str(temp)
                            f_l[b_index][top_num[b_index]][sug_list[c_path][2]] = add_f_l(f_l[b_index][top_num[b_index]][sug_list[c_path][2]],temp)
    ...
    b2_paths = [] #<== This contains all of the paths that we want users in b2 to be able to use
    ...
    for u in range(0,len(b2_u)):
        if b2_u[u] != 'none':
            ...
            for c_path in range(len(sug_list)):
                b_num = 0#2*len(s_nums)-path_num #<== This will change when implemented for base station 2 too
                
                created_flow=0 # <== This will be true if this path has already been created
                for path_num_index in range(len(sug_list[c_path][6])):
                    if int(sug_list[c_path][6][path_num_index]) in b2_paths: #<==This checks if the path # in the list of paths that need to be configured are required for b2 users.
                        print "\n\n ############################### B2 FLOW NEXT  ###############################   \n\n"
                        print "\n sug_list[c_path] = "+str(sug_list[c_path])
                        if sug_list[c_path][0] > 0 and sug_list[c_path][1] > 0:
                            # # Waits for a user input to confirm the network portion has been setup
                            ...
                            # mapping from INPUT ports[path_num][i][0] to OUTPUT ports[path_num][i][1]
                            temp = add_flow_with_meter(switch[sug_list[c_path][2]],sug_list[c_path][3],sug_list[c_path][4],b2_u[u],m_l[b_index][top_num[b_index]][sug_list[c_path][2]][0],sug_list[c_path][5][path_num_index],top_num[b_index],b_num,0)
                            # print str(temp)
                            f_l[b_index][top_num[b_index]][sug_list[c_path][2]] = add_f_l(f_l[b_index][top_num[b_index]][sug_list[c_path][2]],temp)
