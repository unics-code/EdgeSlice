...
import time
import os
import flow_v011_Qiang_SAFE as flow #to get two base stations to work
import numpy as np
import sys
import ctypes
import threading
...
while(testing_var==0):
  ...
  if(top_num==0):
    ...
    print '\n\nMODIFY METER TIME: '+str(time.time()-modify_meter_time)+'\n\n'

    f_l[top_num]=flow.create_topology(b1_u,b2_u,f_l[top_num],switch,m_l[top_num])#creates tbe topology again and readds all of the flows
      ...
      for s in range(0,len(switch)): #goes through all of the switches
        ...
        if (check==-1):
          while((i<len(b2_u)) and (check==-1)):
            ...
            i=i+1
        if(check<>-1):
          flow.modify_meter(switch[s],int(m_l[top_num][check][s][0]),param[h_count+1])
          flow.modify_f_l(f_l[top_num],int(m_l[top_num][check][s][0]))
          # catch flow errors
          i=0
          e_temp = flow.get_E()
          while i<len(e_temp):
