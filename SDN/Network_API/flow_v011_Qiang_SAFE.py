...
max_num_of_flows_per_user=12#TODO THIS NUMBER SHOULD CHANGE TO THE MAX NUMBER OF FLOWS 1 USER MAY HAVE
...
def add_flow_with_meter(*args):
	...
	flow_num=cur_flow=1+max_num_of_flows_per_user*(int(args[4])-1)+num_of_static_flows
	# print '\n\n\n\n '+str(int(args[4]))
	# time.sleep(20)
	#the initial GET
...
def create_topology(*args):
	...
	#This will increase the search of flows number by the number of static flows already on the switch
	if len(b1_u) > 1:
		# print '\n\n\n\n\n\n '+str(b1_u)+'\n\n '+len(b1_u)
		num_of_static_flows=max_num_of_flows_per_user
	else:
		num_of_static_flows=0
	b_num = 1
	for u in range(0,len(b1_u)):
		...
		s_num=0
		temp = add_flow_with_meter(switch[s_num],1,4,b1_u[u],m_l[u][s_num][0],0,switch,b_num)
		f_l = add_f_l(f_l,u,s_num,temp)
		s_num=0
		temp = add_flow_with_meter(switch[s_num],5,8,b1_u[u],m_l[u][s_num][0],0,switch,b_num)
		f_l = add_f_l(f_l,u,s_num,temp)
		s_num=0
		temp = add_flow_with_meter(switch[s_num],9,12,b1_u[u],m_l[u][s_num][0],0,switch,b_num)
		f_l = add_f_l(f_l,u,s_num,temp)

		#core network to base station #1 output route
		s_num=0
		temp = add_flow_with_meter(switch[s_num],12,9,b1_u[u],m_l[u][s_num][0],1,switch,b_num)
		f_l = add_f_l(f_l,u,s_num,temp)
		s_num=0
		temp = add_flow_with_meter(switch[s_num],8,5,b1_u[u],m_l[u][s_num][0],1,switch,b_num)
		f_l = add_f_l(f_l,u,s_num,temp)
		s_num=0
		temp = add_flow_with_meter(switch[s_num],4,1,b1_u[u],m_l[u][s_num][0],1,switch,b_num)
		f_l = add_f_l(f_l,u,s_num,temp)
